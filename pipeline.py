...
from video import get_image_from_bounding_box
...
def pipeline(img, east, vitstr, converter, device):
    ...
    sub_images = [np.array(get_image_from_bounding_box(img_rgb, (box[0][0], box[0][1]), (box[3][0], box[3][1]), (box[2][0], box[2][1]), (box[1][0], box[1][1]))) for box in boxes]
    ...
    return boxes, sub_images, text_preds, boxes_on_image

def process_file(im_path, east, vitstr, converter, device):
    img = cv2.imread(im_path)
    print("read img", img.shape)

    boxes, sub_images, text_preds, img_with_boxes = pipeline(img, east, vitstr, converter, device)

    save_path = rf"results/{im_path.split('/')[-1]}"

    success = cv2.imwrite(save_path, img_with_boxes)
    if success:
...
if __name__ == "__main__":
    ...
    if args.path is not None:
        ...
        for p in im_paths:
            process_file(p, east, vitstr, converter, device)
    else:
